...
export default function StudioTalk() {
  ...
  const [visibleRoots, setVisibleRoots] = useState(PAGE_SIZE);
  ...
  const loadDiscussion = useCallback(async () => {
    ...
    try {
      ...
      setUser(activeUser);
      const { data, error: queryError } = await supabase.from("blog_comments").select("id,user_id,user_name,content,parent_id,created_at,blog_comment_votes(vote,user_id)").eq("post_slug", "community").order("created_at", { ascending: true }).limit(250);
  ...
  const addComment = async (content: string, parentId: string | null = null) => {
    ...
    try {
      ...
      setError(postError instanceof Error && postError.message.includes("wait") ? "You are posting too quickly. Please wait a minute and retry." : "Your message was not posted. Please retry.");
    } finally {
      setPosting(false);
    }
  };
  ...
  const saveEdit = async (comment: Comment) => {
    if (!user || user.id !== comment.user_id || !editText.trim()) return;
    setPosting(true);
    setError("");
    const { error: editError } = await createClient().from("blog_comments").update({ content: editText.trim(), edited_at: new Date().toISOString() }).eq("id", comment.id).eq("user_id", user.id);
    if (editError) setError("Your edit could not be saved. Apply the latest migration if this is the first deployment.");
    else {
      setEditingId(null);
      setEditText("");
      await loadDiscussion();
  ...
  const report = async (comment: Comment) => {
    if (!user) return requireLogin();
    if (!window.confirm("Report this comment for moderator review?")) return;
    setError("");
    const { error: reportError } = await createClient().from("blog_comment_reports").insert({ comment_id: comment.id, reporter_id: user.id, reason: "other" });
    setError(reportError ? (reportError.code === "23505" ? "You have already reported this comment." : "This report could not be submitted.") : "Report submitted for review. Thank you.");
  };
  ...
  const renderComment = (comment: Comment, depth = 0): React.ReactNode => {
    ...
    return <div key={comment.id} className={depth ? `border-l border-white/10 pl-3 sm:pl-5 ${depth < 4 ? "ml-3 sm:ml-6" : ""}` : ""}>
      <article className="rounded-2xl border border-white/5 bg-[#15151c] p-4 transition hover:border-white/10 sm:p-5"><div className="flex gap-3 sm:gap-4">
        ...
        <div className="min-w-0 flex-1"><div className="mb-2 flex flex-wrap items-center gap-2"><strong className="text-sm text-white">u/{comment.user_name}</strong><time dateTime={comment.created_at} className="text-[10px] text-gray-500">{new Date(comment.created_at).toLocaleString()}</time></div>{editingId === comment.id ? <form onSubmit={(event) => { event.preventDefault(); saveEdit(comment); }} className="space-y-2"><label htmlFor={`edit-${comment.id}`} className="sr-only">Edit comment</label><textarea id={`edit-${comment.id}`} value={editText} onChange={(event) => setEditText(event.target.value)} minLength={1} maxLength={5000} rows={3} required autoFocus className="w-full resize-none rounded-lg border border-white/10 bg-black p-3 text-sm outline-none focus:border-[#d4a857]" /><div className="flex gap-2"><button disabled={posting} className="rounded-lg bg-white px-4 py-2 text-[10px] font-bold uppercase tracking-widest text-black disabled:opacity-40">Save</button><button type="button" onClick={() => setEditingId(null)} className="rounded-lg border border-white/10 px-4 py-2 text-[10px] font-bold uppercase tracking-widest text-gray-400">Cancel</button></div></form> : <p className="whitespace-pre-wrap break-words text-sm leading-relaxed text-gray-300">{comment.content}</p>}<div className="mt-4 flex flex-wrap gap-5"><button type="button" onClick={() => user ? setReplyTo(replyTo === comment.id ? null : comment.id) : requireLogin()} className="text-[10px] font-bold uppercase tracking-widest text-gray-500 hover:text-[#d4a857]">Reply</button>{user?.id === comment.user_id ? <><button type="button" onClick={() => { setEditingId(comment.id); setEditText(comment.content); }} className="text-[10px] font-bold uppercase tracking-widest text-gray-500 hover:text-[#d4a857]">Edit</button><button type="button" onClick={() => remove(comment)} className="text-[10px] font-bold uppercase tracking-widest text-gray-600 hover:text-red-400">Delete</button></> : <button type="button" onClick={() => report(comment)} className="text-[10px] font-bold uppercase tracking-widest text-gray-600 hover:text-red-400">Report</button>}</div>
  ...
  return <section id="studio-talk" className="container mx-auto max-w-4xl scroll-mt-28 px-5 sm:px-6"><div className="rounded-[2rem] border border-white/10 bg-[#0c0c10] p-6 shadow-xl sm:p-8 md:p-12">
    ...
    <div className="border-t border-white/5 pt-8">{error && <div role="status" className="mb-6 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-[#d4a857]/20 bg-[#d4a857]/5 p-4 text-sm text-[#d4a857]"><span>{error}</span><button type="button" onClick={loadDiscussion} className="text-[10px] font-bold uppercase tracking-widest text-white">Refresh</button></div>}{!loading && roots.length > 1 && <div className="mb-6 flex gap-2" aria-label="Sort discussions">{(["new", "top"] as const).map((sort) => <button key={sort} type="button" onClick={() => { setSortBy(sort); setVisibleRoots(PAGE_SIZE); }} aria-pressed={sortBy === sort} className={`rounded-full px-4 py-2 text-[9px] font-bold uppercase tracking-widest ${sortBy === sort ? "bg-[#d4a857] text-black" : "border border-white/10 text-gray-500"}`}>{sort}</button>)}</div>}{loading ? <p className="py-10 text-center text-xs uppercase tracking-widest text-gray-600">Loading discussion…</p> : roots.length ? <><div className="space-y-4">{roots.slice(0, visibleRoots).map((comment) => renderComment(comment))}</div>{visibleRoots < roots.length && <button type="button" onClick={() => setVisibleRoots((count) => count + PAGE_SIZE)} className="mt-6 w-full rounded-xl border border-white/10 py-3 text-[10px] font-bold uppercase tracking-widest text-gray-400 transition hover:border-[#d4a857]/40 hover:text-[#d4a857]">Load more discussions</button>}</> : <div className="py-10 text-center"><p className="text-gray-500">No discussions yet.</p>{!user && <Link href="/auth/login" className="mt-3 inline-block text-xs text-[#d4a857]">Be the first to start one</Link>}</div>}</div>
  </div></section>;
}
