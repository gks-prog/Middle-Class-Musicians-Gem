...
export default function HomeContent({ content }: { content: StudioContent }) {
  const portfolioVideos = content.visuals;
    ...
    <div ref={container} className="w-full overflow-hidden">
      <StructuredData
        data={{
          ...
          "@type": content.sections.faq ? ["WebPage", "FAQPage"] : "WebPage",
          ...
          inLanguage: "en-IN",
          mainEntity: content.sections.faq ? faqs.map((faq) => ({
            "@type": "Question",
            name: faq.question,
      ...
      {content.sections.hero ? <HeroCarousel /> : <h1 className="sr-only">Middle Class Musicians — Music Recording Studio in Delhi</h1>}
      {content.sections.reviews && <GoogleReviews reviews={content.reviews} />}

      {/* ECOSYSTEM FUNNEL */}
      {content.sections.ecosystem && <section className="py-32 relative bg-[#07070a]">
      ...
      {content.sections.visuals && portfolioVideos.length > 0 && <section className="py-32 bg-[#0c0c10] border-y border-white/5">
      ...
      {content.sections.process && <section className="relative py-24 sm:py-32">
        <div className="container mx-auto px-6">
      ...
      {content.sections.testimonials && content.testimonials.length > 0 && <VideoTestimonials videos={content.testimonials} />}

      {content.sections.faq && <section className="border-y border-white/5 bg-[#0c0c10] py-24 sm:py-32" id="frequently-asked-questions">
