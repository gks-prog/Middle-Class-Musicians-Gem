...
import type { Song } from "@/lib/cms/types";
...
export default function PortfolioContent({ songs }: { songs: Song[] }) {
  ...
  const openPlayer = (song: Song) => {
    const sameSong = activeSong?.id === song.id;
    if (sameSong) {
      togglePlay();
      return;
    }
            ...
            return (
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="mt-1 block text-[9px] uppercase tracking-[0.18em] text-gray-600 sm:text-[10px]">{song.raw ? "Full Song · Raw + Final" : "Full Song · Final"}</span>
                </span>
          ...
          <div className="pointer-events-auto mx-auto max-w-5xl overflow-hidden rounded-2xl border border-white/10 bg-[#0b0b0e]/95 shadow-2xl shadow-black/60 backdrop-blur-xl">
            ...
            <div className="grid grid-cols-[auto_1fr_auto] items-center gap-3 px-4 pb-3 pt-4 sm:gap-4 md:grid-cols-[auto_minmax(0,1fr)_auto_auto_auto] md:px-6 md:py-4">
              ...
              <div className="col-span-3 row-start-2 flex rounded-xl border border-white/10 bg-black/50 p-1 md:col-span-1 md:col-start-4 md:row-start-1 md:mx-1">
                <button type="button" disabled={!activeSong.raw} onClick={() => switchMix("raw")} aria-pressed={mix === "raw"} className={`min-h-10 flex-1 rounded-lg px-4 py-2 text-[10px] font-bold uppercase tracking-widest transition-all md:flex-none ${mix === "raw" ? "bg-gold text-black" : "text-gray-500 hover:text-white disabled:cursor-not-allowed disabled:opacity-30"}`}>
                  Raw
                </button>
