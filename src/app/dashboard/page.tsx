    ...
    <div className="min-h-screen pb-32 pt-28 sm:pt-32">
      <div className="container mx-auto max-w-6xl px-5 sm:px-6">
        ...
        {activeView === "account" && <aside className="max-w-2xl rounded-[2rem] border border-white/10 bg-[#0c0c10] p-7 sm:p-10">
          ...
          <form onSubmit={saveName} className="space-y-4"><Field label="Artist / display name"><input value={draftName} onChange={(event) => setDraftName(event.target.value)} minLength={1} maxLength={80} required className="form-control" /></Field><Field label="Verified phone"><input disabled value={user?.phone || ""} className="form-control opacity-60" /></Field><button disabled={saving || draftName.trim() === name} className="w-full rounded-xl bg-white py-4 text-[10px] font-bold uppercase tracking-widest text-black transition hover:bg-[#d4a857] disabled:opacity-40">{saving ? "Saving…" : "Save name"}</button></form>
